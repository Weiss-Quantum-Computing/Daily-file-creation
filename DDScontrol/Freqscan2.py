...
FilenameHead = 'Freqscan2'   # filenames will start with this (can include directory, but use forward slash)
...
#~ Frequency0 = 60295883.4722
print(Frequency0)
# used to scan the chirp frequency
#~ StartingFrequency = -0.320e3 #fclock+dfZeeman+40e30.2410
#~ StoppingFrequency=  -0.220e3
...
# Used to measure AC Zeeman shift caused by uwave
#~ StartingFrequency =f40to3P1newH-delta2-0.448e3 #fclock+dfZeeman+40e30.2410
#~ StoppingFrequency=f40to3P1newH-delta2-0.248e3
...
#~ DetectionPulsePower  = P42to31new
#~ DetectionPulsePower  =Pclock_Pi_120us
#~ DetectionPulsePower  = Pclock_Pi_240us
#~ DetectionPulsePower  = 0.16	# // AFP 3ms HSR -6e3   default 0.16 update 20210910   before 0.20
...
#~ DetectionPulseHSR    = -6e3		#-4e3	 # Hz (only used for AFP pulses) 
...
DetectionPulseLength2 = 14		# typ 0.12 
...
#~ ## For one pulse only##
    ...
    for x in xrange(0,DetRepeats):
        writerm.writerow([Frequency[i], \
                        ...
                        DetectionPulseType, \
                         'Measure'])
	writerm.writerow([f42to31new, -6e3, 3.0, P42to31new*64/36, 1, '4,-2 -> 3,-1'])
	writerm.writerow([f4P1to3P2new, -6e3, 3.0, P4P1to3P2new*64/36, 1, '4,1 -> 3,2'])
	writerm.writerow([f3P2to4P3new, -6e3, 3.0, P3P2to4P3new*64/36, 1, '3,2 -> 4,3'])
	...
	#~ writer.writerow([f31to41newH - 16e3, -8e3, 3.0, P42to31new*64/36, 1, '3,-1 -> 4,1'])
	#~ writer.writerow([f3P2to4P2newH, -6e3, 3.0, P42to31new*64/36, 1, '3,2 -> 4,2'])
	...
	#~ writer.writerow([f3P2to4P3newH - 16e3, -8e3, 3.0, P3P2to4P3new*64/36, 1, '3,2 -> 4,3'])
	...
	#~ for y in xrange(0,n):
		#~ writerm.writerow([Frequency[i], \
                        ...
                        #~ DetectionPulseType, \
                        #~ 'Measure'])
	#~ writerm.writerow([f4P1to3P2newH, 0, 0.12,3.406*0, 2, '4,1 -> 3,2'])
	#~ writerm.writerow([f31to41newH, -8e3, 3.0, P42to31new*64/36, 1, '3,-1 -> 4,1'])
	#~ writerm.writerow([f3P2to4P2newH, -8e3, 3.0, P42to31new*64/36, 1, '3,2 -> 4,2'])
...
## For several pulses n pulses (frequency jump) ##
    #~ n=2
    #~ for x in xrange(0,DetRepeats):
	#~ for y in xrange(0,n):
...
#~ # used for scanning the frequency jump 
    #~ for x in xrange(0,DetRepeats):
        #~ writer.writerow([Frequency0, \
                        ...
                        #~ DetectionPulseType, \
                        #~ 'Measure'])
	#~ writer.writerow([Frequency0+Frequency[i], \
                        #~ 0, \
	...
	#~ writerm.writerow([Frequency0, \
                        ...
                        #~ DetectionPulseType, \
                        #~ 'Measure'])
	#~ writerm.writerow([Frequency0+Frequency[i], \
                        #~ 0, \
                        #~ DetectionPulseLength2, \
                        #~ 0, \
                        #~ 3, \
...
#~ # used for checking the coherence to 42 
    #~ for x in xrange(0,DetRepeats):
        #~ writer.writerow([Frequency0, \
                        #~ Frequency[i], \
                        #~ DetectionPulseLength, \
                        ...
                        #~ DetectionPulseType, \
                        #~ 'Measure'])
	#~ writer.writerow([Frequency0 - 210 , \
                        #~ 0, \
                        #~ DetectionPulseLength2, \
                        #~ 0, \
                        #~ 3, \
                        #~ 'Measure'])
	#~ writer.writerow([f4P1to3P2newH, 0, 0.12,3.5, 2, '4,1 -> 3,2'])
	#~ writer.writerow([f4P1to3P2newH, 0, 0.1,0, 2, '4,1 -> 3,2'])
	#~ writer.writerow([f4P1to3P2newH, 0, 0.12,3.5, 2, '4,1 -> 3,2'])
	#~ writer.writerow([Frequency0 - 210 , \
                        #~ 0, \
                        #~ DetectionPulseLength2, \
	...
	#~ writerm.writerow([Frequency0, \
                        ...
                        #~ DetectionPulseType, \
                        #~ 'Measure'])
	#~ writerm.writerow([Frequency0 - 210 , \
                        #~ 0, \
                        #~ DetectionPulseLength2, \
                        #~ 0, \
                        #~ 3, \
                        #~ 'Measure'])
	#~ writerm.writerow([f4P1to3P2newH, 0, 0.12,3.5, 2, '4,1 -> 3,2'])
	#~ writerm.writerow([f4P1to3P2newH, 0, 1,0, 2, '4,1 -> 3,2'])
	#~ writerm.writerow([f4P1to3P2newH, 1, 0.12,3.5, 2, '4,1 -> 3,2'])
	#~ writerm.writerow([Frequency0 - 210 , \
                        #~ 0, \
                        #~ DetectionPulseLength2, \
                        #~ 0, \

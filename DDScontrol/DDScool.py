...
# ------  below modified on 20191120
#Parameters for testing in deeper lattice for 0.5ms pulses
#~ AFPPower0   =1.
#~ AFPPower1   = 1.4
#~ ##~ AFPPower0   =1.4
#~ ##~ AFPPower1   = 2.5
...
f4new
# updated 2022.04.12
FreqM1X = fc+ 17e3   #13.3e3-2e3   #10.2e3	# Hz, -1 sideband
FreqM2X = fc+ 34e3   #27.8e3-2e3  #22.3e3   # Hz, -2 sideband
FreqM1Y = fc+ 14e3	#10.7e3-2e3 #14.9e3    # Hz, -1 sideband
FreqM2Y = fc+ 28e3 	#-2e3 #29.5e3   # Hz, -2 sideband
FreqM1Z = fc+ 14e3   #	12.4e3-2e3  #8.5e3 # Hz, -1 sideband
FreqM2Z = fc+ 28e3   #28.4e3-2e3  #19.1e3 # Hz, -2 sideband


#~ FreqM1X = fc+ 25e3   #13.3e3-2e3   #10.2e3	# Hz, -1 sideband
#~ FreqM2X = fc+ 50e3   #27.8e3-2e3  #22.3e3   # Hz, -2 sideband
#~ FreqM1Y = fc+ 21e3	#10.7e3-2e3 #14.9e3    # Hz, -1 sideband
#~ FreqM2Y = fc+ 42e3 	#-2e3 #29.5e3   # Hz, -2 sideband
#~ FreqM1Z = fc+ 20e3   #	12.4e3-2e3  #8.5e3 # Hz, -1 sideband
#~ FreqM2Z = fc+ 40e3   #28.4e3-2e3  #19.1e3 # Hz, -2 sideband
# ------ above modified on 20191120
...
#~ for i in xrange(0,10): #last 10 cycles only -1 pulse
	...
	#~ writer.writerow([FreqM1X+frec(t+4.2), AFPHSR0, AFPLength,  AFPPower0, 1, 'Y-1 pulse %d'%(i+1)]) # -1 cooling pulse
	#~ writer.writerow([FreqM1Z+frec(t+8.4), AFPHSR0, AFPLength, AFPPower0, 1, 'Z-1 pulse %d'%(i+1)]) # -1 cooling pulse
#~ for i in xrange(0,numcool): # Typical cooling sequence (-2, -1)
	
	#~ writer.writerow([FreqM2X, AFPHSR2, AFPLength, AFPPower2, 1, 'X-2 pulse %d'%(i+1)]) # -2 cooling pulse
	#~ writer.writerow([FreqM1X, AFPHSR1, AFPLength, AFPPower1, 1, 'X-1 pulse %d'%(i+1)]) # -1 cooling pulse
	#~ writer.writerow([FreqM2Y, AFPHSR2, AFPLength,  AFPPower2, 1, 'Y-2 pulse %d'%(i+1)]) # -2 cooling pulse
	#~ writer.writerow([FreqM1Y, AFPHSR1, AFPLength,  AFPPower1, 1, 'Y-1 pulse %d'%(i+1)]) # -1 cooling pulse
	#~ writer.writerow([FreqM2Z, AFPHSR2, AFPLength, AFPPower2, 1, 'Z-2 pulse %d'%(i+1)]) # -2 cooling pulse
	#~ writer.writerow([FreqM1Z, AFPHSR1, AFPLength, AFPPower1, 1, 'Z-1 pulse %d'%(i+1)]) # -1 cooling pulse

#~ for i in xrange(0,15): #last 10 cycles only -1 pulse
	#~ writer.writerow([FreqM1X, AFPHSR1, AFPLength, AFPPower1, 1, 'X-1 pulse %d'%(i+1)]) # -1 cooling pulse
...
copyfile('RAWCool','C:/Users/Yang/Documents/Eclipse/CameraGUI/release/DDScool.txt')
copyfile('RAWCool','C:/Users/Yang/Documents/Eclipse/CameraGUI/debug/DDScool.txt')
copyfile('RAWCool','C:/Users/Yang/Documents/Eclipse/CameraGUI/DDScool.txt')
os.system("del RAWCool")
if DelTempFiles == 1 :
